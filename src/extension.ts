...
import { Definition, buildCommandId, buildStoragePath, getInitializationTargets, mergeLibraryPath, shouldShowOptionalNotifications } from './definitions';
...
const initializationTargets = [
   { definition: mqDefinitions, commandKeyword: 'core.download' as const },
   { definition: mqPluginDefinitions, commandKeyword: 'plugins.download' as const }
];
...
async function initialCheckAndRegisterCommand(context: vscode.ExtensionContext, config: vscode.WorkspaceConfiguration, definition: Definition, commandKeyword: 'core.download' | 'plugins.download') {
   try {
   ...
   const disposable = vscode.commands.registerCommand(buildCommandId(commandKeyword), async () => {
      await checkAndUpdate(context, config, definition);
   });
...
export async function activate(context: vscode.ExtensionContext) {
   ...
   const activeTargets = getInitializationTargets(config, initializationTargets);
   for (const target of activeTargets) {
      await initialCheckAndRegisterCommand(context, config, target.definition, target.commandKeyword);
   }
}
